<template>
  <div class="col" style="max-width: 100%; overflow: scroll">
    <svg v-if="trellis" :height="`${svgHeight}px`" :width="`${svgWidth}px`">
      <line v-for="(edge, index) in trellis.edges" :key="index + 'edge'" v-b-tooltip.hover :title="edgeInfo(edge)"  :x1="nodeX(edge.from)" :y1="nodeY(edge.from)" :x2="nodeX(edge.to)" :y2="nodeY(edge.to)" :stroke="edge.color" stroke-width="5" />
...
export default {
    ...
    methods: {
      ...
      edgeInfo(edge) {
        return `input: ${edge.u}\noutput: ${edge.x} logprob: ${edge.log_g}`
      },
      nodeInfo(node) {
